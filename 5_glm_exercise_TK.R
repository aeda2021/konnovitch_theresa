...
#It can't be normal because you can't have abundance < 0
...
# x-axis is the number of successes; in a dataset of 500 samples with 20 trials per sample and a .5 propabilty of success, would expect around 10 successes
...
ggplot(soay, aes(x = fitness, y = body.size))+
    geom_point()+
...
soay_lm<-lm(body.size ~ fitness, data = soay)
hist(soay_lm$residuals)
...
###There are a lot of 0 values, and a low mean

...
#higher frequencies in the resampling, but generally they are pretty similar; there seems to be a gap
# Q: do the two distributions have the same mean? write code below to find out
mean(pennies_sample$year) #1994.82
mean(pennies_resample$year) #1996.74

# Q: what would happen if you sampled without replacement?
#The distribution looks identical to the first (I used the code # out to double check this thought)
#Without replacement means that we are sampling each year once = the original data set
#The samples aren't independent; what we get the second time is influenced by the first sampling

#pennies_resample <- pennies_sample %>% 
 # rep_sample_n(size = 50, replace = FALSE, reps = 1)
#hist(pennies_resample$year, breaks = 30)
...
#Each time we resample, the mean collected will be representative of the entire population, meaning that we will 
#increase in both central and variant means; the variant means contribute to a pretty spread-out distribution
...
#prediction = less spread out distribution
pennies_resample <- pennies_sample %>% 
...
#still looks pretty normal; still doing this 100 times, even if you're only collecting a few each time
...
pennies_resample_meds <- aggregate(x = pennies_resample$year,
                                    by = list(pennies_resample$replicate),
                                    FUN = median)
hist(pennies_resample_meds$x)
...
bee_prefs <- read.csv("~/Classes/AEDA/konnovitch_theresa/12_bee_prefs.csv")
# Q: are these data tidy?  should they be?
#They are not; I would personally reorganize it so that it is plant species, n_bees, and year.
#I'm not sure if they "should" be...I feel that data should always be tidy? Are there any major issues, besides inconvenience?
#If we made this data tidy, would it mess up the matrix format?
...
#It is just shuffling items in an existing vector = sampling without replacement
#So we set replace = F to be consistent with the definition of a permutation

# check whether column and row sums are maintained
colSums(bee_prefs)
colSums(bee_prefs_perm)
# Q: are column sums maintained?  should they be?
#Yes, and yes because we're permuting in a column, the sum will be same
rowSums(bee_prefs)
rowSums(bee_prefs_perm)
# Q: are row sums maintained?  should they be?
#no - because when you shuffle the column, the row sum will be affected
...
visits <- read_csv("~/Classes/AEDA/konnovitch_theresa/12_visits.csv")
...
pollen <- read_csv("~/Classes/AEDA/konnovitch_theresa/12_pollen.csv")
...
#estimates pollination by summing up all of the visits that included pollination 
#samples both visits pollen, then sums 
# Q: why did we use size=visit_draw?
#you can't pollinate without visiting, so the sample size of pollination should be <= visits
...
#gamma distribution is more evenly distributed 
...
#Gamma, based on AICc, looks best

# Q: write code below to choose a distribution for the pollen data
hist(pollen$consp_grains, breaks = 50)
mean_pol <- mean(pollen$consp_grains)
mean_pol
hist(rpois(501, mean_pol), breaks = 50)
# data seem more skewed than poisson
# let's look at gamma, since that worked before
...
scale1
hist(rgamma(501, shape = shape1, scale = scale1), breaks = 50)
...
bee_mass <- read_csv("~/Classes/AEDA/konnovitch_theresa/12_bee_mass.csv")
glimpse(bee_mass)
...
#talbe with the  mean for each replicate
...
#column stat = difference in mass between the queen and worker bees

# Q: how will you use these numbers? what are we trying to test here?
#we used the means to generate our estimate of the 95CI interval
#testing hypothesis that body mass is the same for each caste
...
#If groups aren't different (null), then difference in mean should be around 0, hist of these means from permutation shows this
#permutation shuffles these, making around 0
...
# I think there is a significant difference between null and data, based on the means
...
obs_stat = 5  # just retyping the numbers we got above
...
530/10000
# p = 0.053 one-sided
# if two-tailed p value, it would be 2x as big

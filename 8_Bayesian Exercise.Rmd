...
crab.mod <- function(){
  for(i in 1:N) {
    y[i] ~ dpois(mu[i], 1/sig.sq)
    mu[i] <- alpha + b1[i] + x1[i]
  }
  alpha ~ dpois(0, 10)
  b1 ~ dpois(0,10)
  x1 ~ dpois(0,10)
...
datList.crabs <- list(
 y <- crabs$salinity, # observations themselves
 N <- length(y),# number of observations
 x <- crabs$burrows # predictor
)
...
data = datList.crabs,
model.file = crab.mod,
parameters.to.save = c('alpha', 'b1', 'x1', 'sigma'), 
n.chains = 3,
n.iter = 20000,
n.burnin = 5000,
n.thin = 3
...
plotPost(fm.crabs.dat$burrows, main = "Posteriors of Mean Crab Burrows")
plotPost(fm.crabs.dat$salinity, main = "Effect of Salinity")
...
datList.ticks <- list(
  ...
  f.area = ticks$forest , # forest area
  f.edge = ticks$edge , # forest edge
  e.green = ticks$evergreen , #evergreen forest
  site = ticks$site , #site
  num.site = length(unique(ticks$site) , # number of sites
)

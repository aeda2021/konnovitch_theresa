...
example_data = rnorm(32, mean(mtcars$wt), sd(mtcars$wt)) #32 is the # data points to draw
hist(example_data)
qqnorm(example_data)
#gives a bunch of plots that show the perfect normal distribution; if it falls
#within that range of distribution
...
#The data is normally distributed, as you can see when comparing it to the 
#example_data and the p-value is significant
...
# Q: the intercept?
# Q: the df for the error?  Why? #degree of freedom 
...
new_augmented_car_mod <- augment(car_mod, interval = 'prediction') # need to specify the optional argument interval here, otherwise defaults to not giving CI
glimpse(augmented_car_mod)
ggplot(new_augmented_car_mod, aes(x = wt, y = mpg)) + 
    geom_point() + 
    geom_line(aes(y = .fitted)) +  # augment uses .whatever for all the new columns it creates to avoid confusion with existing variables
...
iris_mod <- lm(Sepal.Width ~ Species, iris)
...
library(readr)
flowers <- read_csv("~/RUTGERS/CLASSES/AEDA/2021_master/raw-data/3_FlowerColourVisits.csv")
 # substitute your own file path here
...
limpet <- read_csv("~/RUTGERS/CLASSES/AEDA/2021_master/raw-data/3_limpet.csv")  
...
fit <- lm(mpg_z ~ hp_z + wt, data=mtcars)
summary(fit)
